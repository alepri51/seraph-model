...
function Model(seraphDb, type) {
  ...
  var allWhitelistedKeys = function() {
    ...
    return self.fields.concat('_rel',
      [ self.db.options.id ],
  ...
  Object.defineProperty(this, 'type', {
    ...
    set: function(newType) {
        if(Array.isArray(newType)) {
            ...
            this.types = newType.map(type => '`' + type + '`');
...
Model.prototype.touch = function(node, callback) {
  ...
  this.db.query(query, { id: node[this.db.options.id] }, function(err, result) {
    if (err) return callback(err);
    var newUpdated = Array.isArray(result) ? result[0].updated : result.updated;
    node[self.updatedField] = newUpdated;
    callback(null, node);
  });
};
...
Model.prototype.setUniqueKey = function(key, returnOldOnConflict, callback) {
  ...
  this.db.constraints.uniqueness.createIfNone(this.type, key, callback);
};


Model.prototype.save = require('./write').save;
Model.prototype.update = require('./write').update;
...
Model.prototype._findAll = util.promisify(Model.prototype.findAll);
Model.prototype._save = util.promisify(Model.prototype.save);
Model.prototype._update = util.promisify(Model.prototype.update);
Model.prototype._where = util.promisify(Model.prototype.where);
Model.prototype._query = util.promisify(Model.prototype.query);

Model.prototype._findOne = async function(predicate, opts) {
    //predicate[this.db.options.id] && (predicate.id = predicate[this.db.options.id]);
    opts = opts || {};
    opts.restrictToComp = opts.compositions === '*' ? void 0 : opts.compositions ? opts.compositions : void 0;
    
    let found = await this._where(predicate, opts || {});
    return found[0];
}

module.exports = function createSeraphModel(db, type, schema) {
    let model = new Model(db, type);
    model.useTimestamps();
    
    schema && (model.schema = schema)
    schema && (model.fields = Object.keys(schema));
    return model;
};

module.exports.modelPrototype = Model.prototype;
