...
function assembleReadQuery(opts) {
  ...
  if (opts.rootId != null || opts.query != null) {
    query.push('WHERE (' + opts.varName + ':' + (this.types || ('`' + this.type + '`')) + ')');
  } else {
    query.push('MATCH (' + opts.varName + ':' + (this.types || ('`' + this.type + '`')) + ')');
  }
  ...
  if (opts.include) {
    Object.keys(opts.include).forEach(function(include) {
      ...
      query.push('OPTIONAL MATCH (' + opts.varName + ')' + rel +
                 '(__sm_inc_' + include + ':' +  (includeOpts.model.types || ('`' + includeOpts.model.type + '`')) + ')');
    });
  }
...
module.exports = {
  read: function(node, opts, callback) {
    ...
    this.db.query(query, opts.params)
    .then(data => {
